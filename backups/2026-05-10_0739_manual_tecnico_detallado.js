...
<span class="kw">function</span> <span class="fn">logAllPlcsProduction</span>() {
  ...
  fs.<span class="fn">appendFileSync</span>(<span class="str">'data/production.jsonl'</span>, <span class="kw">JSON</span>.<span class="fn">stringify</span>(snapshot) + <span class="str">'\n'</span>);
      ...
      <!-- SECCIÓN 12: GESTIÓN DE TIEMPOS -->
      <section id="tiempos">
        <h2 id="sec-12">12. Gestión de Tiempos y Ciclos de Datos</h2>
        <p>Es fundamental distinguir entre los tres niveles de temporización del sistema:</p>
        
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-top: 20px;">
          <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px; border-top: 3px solid #06b6d4;">
            <h4 style="margin-top:0; color:#06b6d4;">1. Ciclo PLC</h4>
            <p style="font-size: 0.9em;">Definido en el bloque <code>FB_Sim_Produccion</code> (TIA Portal). Determina la velocidad a la que se incrementan los contadores y cambian las temperaturas reales en la máquina.</p>
          </div>
          <div style="background: rgba(124,58,237,0.1); padding: 15px; border-radius: 8px; border-top: 3px solid #7c3aed;">
            <h4 style="margin-top:0; color:#7c3aed;">2. Registro Servidor</h4>
            <p style="font-size: 0.9em;">Es el <strong>Reloj Maestro</strong>. Se configura desde la página de Analítica (Aplicar). Decide cada cuánto tiempo se guarda un punto en la base de datos (Snapshots).</p>
          </div>
          <div style="background: rgba(16,185,129,0.1); padding: 15px; border-radius: 8px; border-top: 3px solid #10b981;">
            <h4 style="margin-top:0; color:#10b981;">3. Refresco Web</h4>
            <p style="font-size: 0.9em;">Es puramente visual. La página de Analítica se recarga automáticamente cada 5 minutos para mostrar los nuevos datos que el servidor haya capturado.</p>
          </div>
        </div>
      </section>
    </div>
