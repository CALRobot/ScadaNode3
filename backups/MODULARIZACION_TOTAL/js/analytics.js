...
async function loadData() {
  ...
  try {
    const res = await fetch(`/api/production/history?plc=${plc}`);
    const raw = await res.json();
    // Filtramos por el límite seleccionado en el combo
    const n = parseInt(limit) || 200;
    allData = raw.slice(-n); // Cogemos solo los últimos 'n' registros
    renderAll();
  } catch (e) {
