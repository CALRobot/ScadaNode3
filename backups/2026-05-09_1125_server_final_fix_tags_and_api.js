...
// --- LOGGING PRODUCCIÓN ---
let productionInterval = 10 * 60 * 1000; // 10 minutos por defecto
...
function logProduction() {
    ...
    Object.keys(plcControllers).forEach(id => {
        ...
        if (ctrl.isConnected()) {
            ...
            console.log('[LOG] Datos guardados en production.jsonl');
        } else {
            console.warn(`[LOG] PLC ${id} no conectado, no se puede guardar snapshot.`);
        }
    });
}
...
function logAlarms(id, currentTags) {
    ...
    const alarmTags = tagsPlc1.filter(t => t.cat === 'Alarma');
...
function createPlcController(device, tagsDef) {
    ...
    const connect = () => {
        console.log(`[MODBUS] Conectando a ${device.id} en ${device.host}:${device.port}...`);
        socket.connect({ host: device.host, port: device.port });
    };

    socket.on('connect', () => { 
        console.log(`[MODBUS] ${device.id} CONECTADO.`);
        isConnected = true; 
        poll(); 
    });

    socket.on('close', () => { 
        console.log(`[MODBUS] ${device.id} DESCONECTADO. Reintentando en 5s...`);
        isConnected = false; 
        setTimeout(connect, 5000); 
    });

    socket.on('error', (err) => { 
        console.error(`[MODBUS] Error en ${device.id}:`, err.message);
        isConnected = false; 
    });
...
server.listen(4000, () => { 
    ...
    startProductionLogging();
    // Primer log tras un pequeño delay para asegurar conexión
